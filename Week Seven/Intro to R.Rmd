...
```{r}
# This is how you make a comment (doctrings do not work)
```
...
```{r}
print("John ate 17 cookies today...")
```
...
#string <- sprintf("%s's last name is %s", name, last_name)
```

```{r}
#sub('Martin', 'Doroh', string)
...
```{r}
# Generating a sequence from 5 to 15, inclusive of 15,
# By a step size of .3
...
```{r}
# Get the first and third rows as well as the first two columns
df[c(1,3), 1:2]
```
...
1.  Create an array of two 3x3 matrices from the given vectors.
    ...
    x <- array(c(vec1, vec2), dim = c(3,3,2))
    x
...
5.  Create two strings with your first and last name, respectively. Then concatenate them, and print the new value as "My name is \<your newly created concatenated variable will go here\>"
    ...
    hello <- sprintf("My name is %s", full_name)
    hello
